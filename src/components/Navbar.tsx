...
function ResponsiveAppBar({ userInfo }: Props) {
  ...
  return (
    <AppBar position="static">
      {isAlert && <CustomAlert text="Successfully logout" status="success" />}
          ...
          <Box sx={{ flexGrow: 0 }}>
            <Tooltip title="Open settings">
              <IconButton onClick={handleOpenUserMenu} sx={{ p: 0 }}>
                {(authCtx?.user || userInfo) && <Avatar alt="" src="/" />}
              </IconButton>
            </Tooltip>
            <Menu
              sx={{ mt: "45px" }}
